...
def CARTDecisionTree ( X_train, y_train, X_test, y_test, k ):
	...
	print( 'accuracy_score：', accuracy_score( y_test, y_pred ) )
	params={ 'ccp_alpha'        : [ 0.01, 0.1, 0.05], 'class_weight': ["balanced"], 'criterion': ['gini','entrop'],
...
if __name__ == "__main__":
	...
	# 	df[name]=info
	# print(df)
	# df.to_csv("/Users/gyw/Desktop/Project/慢阻肺合并症-gyw/05模型评分.csv",header=True,index=True)
	# models = [rf,xgb,mlp,gbm,knn,dt,gnb,cart,ada]
	models=[rf]
	for model in models:
