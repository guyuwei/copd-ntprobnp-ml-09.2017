...
def preprocess_arrhythmia_data(df):
    """
    预处理心律失常数据，将 RBBB 和 LBBB 合并为 IVB。
    :param df: 包含心律失常类型列的数据框
    :return: 预处理后的数据框
    """
    # 创建新的 IVB 列
    df["IVB"] = df[["RBBB", "LBBB"]].max(axis=1)

    # 删除原始的 RBBB 和 LBBB 列
    df = df.drop(columns=["RBBB", "LBBB"])
...
if __name__ == "__main__":
    ...
    # 预处理心律失常数据
    copd_patients = preprocess_arrhythmia_data(copd_patients)

    # 更新心律失常类型列表
    AF_index = ["IVB", "AF", "SVT", "PAC", "PVC"]
    ...
    # group_analysis(copd_patients, AF_index)  # 按疾病分类的开销
